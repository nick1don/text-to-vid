...
Uses a D-ID stock presenter (no custom photo) and a distinct background style.
...
ELEVENLABS_KEY = (os.environ.get("ELEVENLABS_API_KEY")
                  or os.environ.get("elevenlabs_api_key") or "")
D_ID_KEY       = (os.environ.get("D_ID_API_KEY")
                  or os.environ.get("d_id_api_key") or "")

if not ELEVENLABS_KEY: sys.exit("Error: set ELEVENLABS_API_KEY")
if not D_ID_KEY:       sys.exit("Error: set D_ID_API_KEY")

# ── paths ──────────────────────────────────────────────────────────────────────
OUTPUT_DIR = "./output"
os.makedirs(OUTPUT_DIR, exist_ok=True)
AUDIO_PATH = f"{OUTPUT_DIR}/demo_voice.mp3"
WAV_PATH   = f"{OUTPUT_DIR}/demo_voice.wav"
ANIM_PATH  = f"{OUTPUT_DIR}/demo_head.mp4"
OUT_PATH   = f"{OUTPUT_DIR}/demo.mp4"
...
One command. One output file. Built with Claude Code.\
"""

ELEVENLABS_VOICE_ID = "nPczCjzI2devNBz1zQrb"
VOICE_SETTINGS      = {"stability": 0.55, "similarity_boost": 0.75}

# ── background — distinct style from generate.py ───────────────────────────────
W, H   = 1280, 720
FPS    = 30
BG_COLOR     = (4,  12,   4)    # deep green-black
ACCENT_COLOR = (0,  220, 100)   # bright green
GRID_COLOR   = (10,  22,  12)
...
def load_font(size, mono=False):
    paths = (
        ...
        if not mono else
        ["/System/Library/Fonts/Menlo.ttc", "/System/Library/Fonts/Courier New.ttf"]
    )
    for p in paths:
        if os.path.exists(p):
...
def generate_speech():
    words, current, parts = SPOKEN_TEXT.split(), [], []
    for word in words:
        current.append(word)
...
def generate_talking_head():
    """Uses a D-ID stock presenter — no custom photo required."""
    auth = f"Basic {D_ID_KEY}"

    print("  uploading audio...")
    with open(WAV_PATH, "rb") as f:
        r = requests.post("https://api.d-id.com/audios",
                          files={"audio": ("demo_voice.wav", f, "audio/wav")},
                          headers={"Authorization": auth})
    r.raise_for_status()
    audio_url = r.json()["url"]

    print("  creating clip (stock presenter)...")
    r = requests.post(
    ...
    print(f"  polling (id: {clip_id})...")
    for attempt in range(72):
        poll   = requests.get(f"https://api.d-id.com/clips/{clip_id}",
...
def main():
    ...
    if not os.path.exists(ANIM_PATH):
        print("Step 1: Generating talking head (stock presenter)...")
